...
import { stevenInterviewWithOpenAI } from "./openaiService";

const genAI = new GoogleGenerativeAI(API_KEYS.GEMINI_STEVEN);
...
class StevenInterviewService {
  ...
  /**
   * Initialize STEVEN's personality and context
   */
  private getSystemPrompt(context: InterviewContext): string {
    return `You are STEVEN, a professional AI interviewer conducting a ${context.role} interview.
...
Respond naturally as a human interviewer would.`;
  ...
  async startInterview(candidateName: string, role: string, isFirstTime: boolean): Promise<{ greeting: string; context: InterviewContext }> {
    console.log('🚀 STEVEN: Starting interview...');
    ...
    console.log('🔧 STEVEN: Initializing conversation session...');
    const provider = getAIProvider();
    
    try {
      if (provider === 'openai') {
        // OpenAI is stateless — we track context manually
        console.log('🟢 STEVEN: Using OpenAI backend proxy');
        ...
        this.conversationSession = await this.model.startChat({
          history: [
            ...
            {
              role: "model",
              parts: [{ text: "I understand. I'm STEVEN, ready to conduct this interview professionally and naturally." }],
            },
          ],
        });
      }
      console.log('✅ STEVEN: Conversation session initialized');
    } catch (error) {
      console.error('❌ STEVEN: Failed to initialize session:', error);
      throw new Error('Failed to initialize AI interview session');
    }

    // Generate greeting
    const greeting = `Hi ${candidateName}, I'm STEVEN, your AI interviewer today. ${
      isFirstTime 
    ...
    console.log('💬 STEVEN Greeting:', greeting);
  ...
  async generateNextQuestion(
    ...
    console.log('🎯 STEVEN: Generating next question...');
    ...
    try {
      if (!this.conversationSession) {
        console.error('❌ STEVEN: No conversation session found!');
...
- Be brief and direct`;
      ...
      try {
        if (provider === 'openai') {
          ...
          const systemPrompt = this.getSystemPrompt(context);
          response = await stevenInterviewWithOpenAI(
            'question',
            systemPrompt,
            prompt,
        ...
        console.log('✅ STEVEN Response:', response);
      ...
      console.error('❌ STEVEN: Error generating next question:', error);
  ...
  private shouldContinueInterview(context: InterviewContext, lastResponse: string): boolean {
    // Check if STEVEN is closing the interview
  ...
  async generateFeedback(context: InterviewContext): Promise<{
    ...
    const prompt = `
As STEVEN, provide final interview feedback for ${context.candidateName}.

Conversation history:
${context.conversationHistory
  .map(msg => `${msg.role === 'candidate' ? 'Candidate' : 'STEVEN'}: ${msg.content}`)
  .join('\n')}
...
Format as JSON:
{
  ...
  async handleClosingQuestion(question: string, context: InterviewContext): Promise<string> {
    const prompt = `The candidate asked: "${question}". Respond as STEVEN would, providing helpful information about the role or company. Keep it concise (2-3 sentences).`;
    ...
    try {
      ...
      if (provider === 'openai') {
        const systemPrompt = this.getSystemPrompt(context);
        return await stevenInterviewWithOpenAI(
          'closing',
          systemPrompt,
          prompt,
          context.conversationHistory
...
export const stevenService = new StevenInterviewService();
