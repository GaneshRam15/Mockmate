...
export const generatePDF = (interview: InterviewSession): void => {
  ...
  doc.text("MockMate Interview Report", 20, 20);
  ...
  if (interview.aborted) {
    ...
    doc.save(`MockMate_${interview.roleName}_ABORTED_${formattedDate.replace(/\s/g, '_')}.pdf`);
    return;
  }
  ...
  doc.text("Generated by MockMate - AI-Powered Interview Trainer", 20, yPosition);
  
  // Save PDF
  doc.save(`MockMate_${interview.roleName}_Interview_${formattedDate.replace(/\s/g, '_')}.pdf`);
};
