...
/**
 * STEVEN interview via OpenAI
 */
export const stevenInterviewWithOpenAI = async (
  action: string,
  systemPrompt: string,
  message: string,
  conversationHistory: Array<{ role: string; content: string }>
  ...
  try {
    const data = await postJSON<{ success: boolean; response: string }>('/steven', {
      action,
      systemPrompt,
      message,
      conversationHistory,
    ...
    console.error('❌ OpenAI STEVEN call failed:', error.message);
    throw error;
  }
};
