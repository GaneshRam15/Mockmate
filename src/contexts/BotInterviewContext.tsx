...
import { InterviewContext as StevenContext } from '../utils/stevenService';
...
export interface BotInterviewState {
  ...
  stevenContext: StevenContext | null;
...
interface BotInterviewContextType {
  ...
  agreeToTerms: () => void;
  updateStevenContext: (context: StevenContext) => void;
  addMessage: (speaker: 'steven' | 'candidate', text: string) => void;
  setStevenSpeaking: (speaking: boolean) => void;
...
export function BotInterviewProvider({ children }: { children: React.ReactNode }) {
  ...
  const updateStevenContext = useCallback((context: StevenContext) => {
    setState(prev => ({ ...prev, stevenContext: context }));
  }, []);

  const addMessage = useCallback((speaker: 'steven' | 'candidate', text: string) => {
    setState(prev => ({
      ...prev,
      conversationLog: [
  ...
  const setStevenSpeaking = useCallback((speaking: boolean) => {
    setState(prev => ({ ...prev, isStevenSpeaking: speaking }));
  }, []);
